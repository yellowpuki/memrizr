...
const (
	...
	Conflict        Type = "CONFLICT"        // Already exist (eg, create account with existent email) - 409
...
// Error satsfies standard error interface
...
func NewConflict(name string, value string) *Error {
	return &Error{
		Type:    Conflict,
		Message: fmt.Sprintf("resource: %v with value: %v alredy exists", name, value),
	}
}
...
func NewInternal() *Error {
	return &Error{
		Type:    Internal,
		Message: fmt.Sprintf("Internal server errror."),
	}
}
...
func NewPayloadTooLarge(maxBodySize int64, contentLength int64) *Error {
	return &Error{
		...
		Message: fmt.Sprintf("Max payload size of %v exeeded. Actual payload size: %v", maxBodySize, contentLength),
	}
}
